  ...
  computed: {
    ...
    filteredEvents() {
      if (this.focusedEventIndex) {
        return [this.events.find(event => event.index === this.focusedEventIndex)];
      }
  ...
  methods: {
    filterByName(events) {
      if (this.filterName.length > 0) {
        return events.filter(event => this.filterName.includes(event.tournamentname));
      }
      return events;
    },
    ...
    filterByLocation(events) {
      if (this.filterLocation.length > 0) {
        return events.filter(event => this.filterLocation.includes(event.location));
      }
      return events;
    },
    filterByType(events) {
